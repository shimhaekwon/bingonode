// app.js — 통합: data.js + script.js (Client-only UI Lotto Viewer)
(() => {
  // ===== 고정값 =====
  ...
  function enableColumnHoverBand(tableHost) {
    ...
    tableEl.addEventListener("mousemove", (e) => {
      ...
      const col =
        Array.prototype.indexOf.call(cell.parentNode.children, cell) + 1;
      setHover(col);
    });
  ...
  function renderDataViewer() {
    ...
    const rows = CLEANED;
    let html =
      "<table><thead><tr><th class='col-radio'>선택</th><th>회차</th><th colspan='6'>당첨번호</th><th>보너스</th></tr></thead><tbody>";
    ...
    host
      .querySelectorAll('input[type="radio"][name="roundPick"]')
      .forEach((radio) => {
    ...
    host.addEventListener("click", (e) => {
      ...
      host
        .querySelector(
          `input[type="radio"][name="roundPick"][value="${tr.dataset.round}"]`
        )
        ?.click();
    });
  }
  ...
  async function renderAll() {
    ...
    try {
      ...
      const sets = (srv.sets || []).map((s) =>
        (s.numbers || []).slice().sort((a, b) => a - b)
      );
      ...
      for (let i = 1; i <= 45; i++) {
        ...
        html += `<td class="${inSet ? "highlight-purple" : ""}">${
          inSet ? 1 : 0
        }</td>`;
      }
      html += "</tr>";
      ...
      for (let i = 1; i <= 45; i++) {
        ...
        html += `<td class="${isNX ? "highlight-nonexpose" : ""}">${
          isNX ? 1 : 0
        }</td>`;
      }
      html += "</tr>";
      ...
      sets.forEach((arr, i) => {
        ...
        line.innerHTML = `세트 ${i + 1}: <span class="nums">${arr
          .map(
            (n) => `<span class="chip ${lottoColorClass(n, "chip")}">${n}</span>`
          )
          .join("")}</span>`;
        setsHost.appendChild(line);
      });

      // Type Rate 패널은 서버에서 통계를 제공하지 않으므로 메시지
      if (el.typePanel) {
  ...
  /* ===================== 외부 이벤트 호환 ===================== */
  // 만약 다른 스크립트가 선행하여 window.numChosen을 채우고
  // 'numChosen:ready'를 발생시키면, 안전하게 초기 렌더만 수행
  document.addEventListener("numChosen:ready", () => {
    safeAfterDOM(() => {
      try {
        if (!el.applyRound) return; // DOM 아직이면 무시
        prepareDataOnce();
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    // 높이 조절 핸들 초기화
    initRealDataHeightControl();

    // 데이터 준비
    try {
      if (Array.isArray(window.numChosen) && window.numChosen.length > 0) {
        appendLog("사전 로드된 Real Data 사용 …");
      } else {
        appendLog("서버에서 Real Data 로드 …");
        window.numChosen = await fetchAllBingoAsNumChosen(1000);
        // 호환성: 외부에서 이 이벤트를 기대할 수 있으니 발행
    ...
    // 미리보기 타입 변경: 현재 서버 예측에는 영향 없음. UI만 갱신.
    ...
    el.pasteDataBtn?.addEventListener("click", () => {
      const hint =
        '한 줄에 "회차, n1, n2, n3, n4, n5, n6, bonus" (쉼표/공백 모두 허용). 여러 줄 붙여넣기 가능.';
      ...
      for (const line of lines) {
        ...
        if (cols.length >= 7) {
          ...
          if (
            ...
            [n1, n2, n3, n4, n5, n6].every((v) => Number.isFinite(v))
          ) {
            out.push([
              round,
              n1,
      ...
      // (원하시면 bulk 업로드 API 확장으로 서버 DB 반영 가능)
      ...
      try {
        ...
        renderAll();
        appendLog(
          `붙여넣기 완료: ${out.length}행 (서버 예측에는 반영되지 않습니다)`,
          "warn"
        );
      } catch (e) {
        console.error(e);
